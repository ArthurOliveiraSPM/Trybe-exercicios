...
const getBestClass = (acc, course) => (acc.grade > course.grade) ? acc : course;


const reportBest = (students) => students.map((student) => {
  return {
    name: student.name,
    course: student.courses.reduce(getBestClass).name,
    couseGrade: student.courses.reduce(getBestClass).grade
  };
});
